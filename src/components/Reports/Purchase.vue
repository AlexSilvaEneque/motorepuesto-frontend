<script setup lang="ts">
    ...
    const filtered = async () => {
        await composable.getDataPurchaseRangeDate(date1.value, date2.value)
        const {purchase, money} = composable.responsePurchase.value

        if (Object.keys(purchase).length > 0) {
            labels1.value = purchase.map((item:any) => item._id)
            data1.value = purchase.map((item:any) => item.total)
